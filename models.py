...
class DeepInfoMax(nn.Module):
    ...
    def __init__(self, nb_samples=1, conv_layers_sizes = [20,10,15,10,5,12],mlp_layers_size = [25,10], emb_size = 10,data_dir ='.'):
        ...
        dim = [(conv_layers_size[1]*(27-conv_layers_size[2]+1))+self.emb_size] + layers_size # Adding the emb size*out_channels
...
def get_model(opt, inputs_size, model_state=None):
    ...
    if opt.model == 'CNN':
        ...
        model = model_class(conv_layers_size = opt.cnn_layers, mlp_layers_size=opt.layers_size, out_channels=opt.out_channels, emb_size=opt.emb_size, data_dir = opt.data_dir)
    elif opt.model == 'classifier':
        model_class = CNNClassifier
        model = model_class(conv_layers_size = opt.cnn_layers, mlp_layers_size=opt.layers_size, out_channels=opt.out_channels, emb_size=opt.emb_size, class_number = opt.nbclasses, data_dir = opt.data_dir)
    #elif opt.model == 'ae':
